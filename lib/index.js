  ...
  context.inject(['systemPrompt'], (scope) => scope.systemPrompt.context({
    ...
    text: (assemblyContext) => {
      try {
        ...
        return `本会话的拖放文件存放在 ${sessionDir}/ 目录；用户消息中形如「[附件: 文件名](绝对路径)」或「[网页资源: 文件名](绝对路径)」的 markdown 链接是拖进对话框的内容，括号内即文件在磁盘上的绝对路径，直接用该路径读取文件本身。形如「[文件夹: 名称](绝对路径)」的是文件夹引用（未上传，指向磁盘原位置），直接用 fs 工具按该路径列出并读取其中的文件。`
      } catch {
        return ''
      }
    },
