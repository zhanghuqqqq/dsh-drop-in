...
const CSS = `
.ddi-overlay{position:fixed;inset:0;z-index:2147483000;display:none;align-items:center;justify-content:center;background:rgba(15,23,42,.42);backdrop-filter:blur(1.5px);pointer-events:none}
.ddi-overlay.ddi-visible{display:flex}
...
function ensureOverlay() {
  ...
  // Visibility is driven purely by the .ddi-visible class (CSS display toggle);
  // never use the hidden attribute — the author rule would override its UA style.
...
function overlayFor(kind) {
  if (kind.kind === 'folders') {
    const n = kind.count > 0 ? `${kind.count} 个文件夹` : '文件夹'
    return `${n} · 添加为路径引用（agent 直接读取原位置）`
  }
  if (kind.kind === 'files') {
...
function showOverlay(kind) {
  ...
  overlayEl.classList.add('ddi-visible')
}

function hideOverlay() {
  if (overlayEl) overlayEl.classList.remove('ddi-visible')
}
...
function classifyDragOver(dt) {
  ...
  if (types.includes('Files')) {
    ...
    const directories = items.filter((it) => it.webkitGetAsEntry?.()?.isDirectory)
    // Folders are takeover now: the plugin resolves their disk path through the
    // Desktop bridge and inserts a reference the agent can walk with fs tools.
    // Letting them through would hit the native image pipeline, which rejects
    // them with its "PNG/JPG/WebP/GIF only" toast.
    if (directories.length > 0) return { kind: 'folders', count: directories.length }
...
function classifyDrop(dt) {
  ...
  if (files.length > 0) {
    ...
    // Resolve folder disk paths NOW, synchronously: DataTransferItemList
    // (webkitGetAsEntry / getAsFile) is only valid inside the drop handler
    // before the first await. The Desktop bridge maps a folder File to its
    // absolute path so the agent can walk the original folder in place.
    ...
    for (const it of items) {
      ...
      const paired = files[fileIdx]
      fileIdx += 1
      if (entry?.isDirectory) {
        const folderFile = (it.getAsFile() ?? paired) || null
        let path = ''
        if (folderFile && bridge && typeof bridge.getPathForFile === 'function') {
          try { path = String(bridge.getPathForFile(folderFile)).trim() } catch { path = '' }
        }
        folders.push({ name: folderFile?.name || entry.name || 'folder', path, resolved: path.length > 0 })
      } else if (paired) {
        plainFiles.push(paired)
      }
    }
    // Pure images with no folders stay on the native vision-attachment path.
    if (folders.length === 0 && plainFiles.every((f) => String(f.type ?? '').startsWith('image/'))) {
      return { kind: 'passthrough' }
    }
    if (plainFiles.length > MAX_FILES_PER_DROP) {
      return { kind: 'too-many', count: plainFiles.length, folders }
    }
    return { kind: 'files', files: plainFiles, folders }
  }
...
function onDragEnter(event) {
  // Blind DSH's native attachment overlay for takeover kinds. Its
  // ComposerAttachments counts dragenter events in a bubble-phase listener
  // and only resets that counter in its own bubble-phase drop handler —
  // which our capture-phase drop already blocks, leaving its
  // "images here" overlay stuck on screen forever. Stopping the dragenter
  // from propagating means its counter never increments and its overlay
  // never shows for takeover kinds. Passthrough kinds (pure images /
  // folders) still reach it untouched.
  const kind = classifyDragOver(event.dataTransfer)
...
function onDragOver(event) {
  ...
  // Same blinding as onDragEnter: keep the native attachment pipeline from
  // reacting to a drag we own (it would only fight us over dropEffect and
  // its overlay state).
  event.stopPropagation()
  dt.dropEffect = 'copy'
  showOverlay(kind)
...
async function onDrop(event) {
  ...
  event.preventDefault()
  // Take over the drop: stop propagation so DSH's native attachment handler
  // never sees the event (it would pop its "images only" toast for non-image files).
  event.stopPropagation()
  ...
  if (kind.kind === 'too-many') {
    if (kind.folders?.length) handleFolders(st, kind.folders)
  ...
  try {
    if (kind.folders?.length) handleFolders(st, kind.folders)
    if (kind.kind === 'files') {
      if (kind.files?.length) await handleFiles(st, kind.files)
      else if (!kind.folders?.length) return
...
/** Insert absolute-path folder references resolved from the Desktop bridge. */
function handleFolders(st, folders) {
  const resolved = folders.filter((f) => f.resolved)
  const failed = folders.filter((f) => !f.resolved)
  if (resolved.length > 0) {
    insertIntoComposer(st, resolved.map((f) => `[文件夹: ${f.name}](${f.path})`).join('\n'))
    toast(resolved.length === 1 ? `已添加文件夹 ${resolved[0].name}` : `已添加 ${resolved.length} 个文件夹引用`)
  }
  if (failed.length > 0) {
    toast('桌面版无法读取该文件夹的磁盘路径，已跳过；可将文件夹直接拖到左侧工作区面板收养', 'err')
  }
}

function onDragLeave(event) {
  // Hide only when the drag left the window entirely (nothing is being
  // entered). relatedTarget is non-null while moving between elements,
  // so the overlay does not flicker inside the page.
...
function onDragEnd(event) {
  // In-document drag finished (drop completed or cancelled) — hide at once.
  // OS file drags have no in-page source and stay covered by drop/timer.
  clearTimeout(hideTimer)
  hideOverlay()
}
